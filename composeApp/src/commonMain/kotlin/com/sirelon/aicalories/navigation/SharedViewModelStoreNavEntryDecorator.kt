 ...
 *
 * Copied from https://github.com/android/nav3-recipes (sharedviewmodel recipe), package renamed.
 */
 ...
 *
 * If the `NavEntry` specifies that it has a parent in its metadata (via [parent]), the parent's
 * [ViewModelStoreOwner] will also be supplied via [LocalSharedViewModelStoreOwner], allowing the
 * entry to access both its own ViewModels and its parent's ViewModels.
 *
 * @param [viewModelStoreProvider] The [ViewModelStoreProvider] scoped to the parent
 *   [ViewModelStoreOwner]
 */
class SharedViewModelStoreNavEntryDecorator<T : Any>(
    ...
    companion object {
        /**
         * Use this function to specify a `NavEntry`'s parent. The parent's [ViewModelStoreOwner]
         * will be supplied via [LocalSharedViewModelStoreOwner].
         */
