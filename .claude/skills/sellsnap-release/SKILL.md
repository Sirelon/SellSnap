...
Bump `version.properties`, translate the release notes into all supported languages,
write them where each store's fastlane action actually reads them from, then run
`scripts/ship.sh` to build and upload. Fully autonomous once started — no pause before
...
## 2. Work out the version numbers
...
## 3. Draft the English "What's New" first
...
## 6. Promotional text — leave it alone unless asked
